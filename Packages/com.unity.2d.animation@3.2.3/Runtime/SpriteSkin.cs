...
namespace UnityEngine.U2D.Animation
{
    ...
    {
        public delegate void SkinProcessingEventHandler(ref NativeArray<byte> e);
        public event SkinProcessingEventHandler OnSkinChanging;
...
#endif
        {
            ...
            if (isValid && !batchSkinning && this.enabled && (this.alwaysUpdate || this.spriteRenderer.isVisible))
            {
                ...
                if (spriteVertexCount > 0)
                {
                    ...
                    OnSkinChanging?.Invoke(ref inputVertices);
        ...
        public bool DeformedVerticesAvailable
        {
            get
            {
                if(isValid && !batchSkinning && this.enabled && (this.alwaysUpdate || this.spriteRenderer.isVisible))
        ...
        public ref NativeArray<byte> DeformedVertices
        {
            get
            {
                var spriteVertexCount = sprite.GetVertexStreamSize() * sprite.GetVertexCount();
                ref var inputVertices = ref GetDeformedVertices(spriteVertexCount);
                SpriteSkinUtility.Deform(sprite, gameObject.transform.worldToLocalMatrix, boneTransforms, ref inputVertices);
                SpriteSkinUtility.UpdateBounds(this, inputVertices);

                return ref inputVertices;
            }
        }

        public void FinishDeformation(ref NativeArray<byte> inputVertices)
        {
            InternalEngineBridge.SetDeformableBuffer(spriteRenderer, inputVertices);
        }
